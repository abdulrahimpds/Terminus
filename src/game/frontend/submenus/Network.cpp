...
namespace YimMenu::Submenus
{
	    ...
	    Submenu::Submenu("Network")
	{
		...
		infoSpoofingGroup->AddItem(std::make_shared<ImGuiItem>([=] {
			...
			ImGui::SameLine();
			if (ImGui::Button("?##rand_ip"))
			{
				std::random_device rd; 
				std::mt19937 gen(rd());
				std::uniform_int_distribution<> dist(1, 255);
				for (int i = 0; i < 4; ++i)
					g_SpoofingStorage.spoofedIP[i] = dist(gen);
			}
			if (ImGui::IsItemHovered())
				ImGui::SetTooltip("Generate random IP");
			...
			ImGui::SameLine();
			if (ImGui::Button("?##rand_rid"))
			{
				std::random_device rd;
				std::mt19937_64 gen(rd());
				std::uniform_int_distribution<uint32_t> dist(1000u, 999999999u);
				g_SpoofingStorage.spoofedRID = static_cast<long long>(dist(gen));
			}
			if (ImGui::IsItemHovered())
				ImGui::SetTooltip("Generate random RID");
