...
namespace YimMenu::Hooks
{
	...
	bool Protections::ReceiveNetMessage(void* a1, rage::netConnectionManager* ncm, rage::netConnection::InFrame* frame)
	{
		...
		{
			// pass to original handler to match base behavior for unknown frames
			return CallOrig_SEH(orig, a1, ncm, frame);
		}
		...
		if (__ym_should_log && Features::_LogPackets.GetState())
		{
			...
			// route-change protection disabled due to join isolation false positives
				...
				// peer-originated session-control filtering disabled due to join/empty session false positives
